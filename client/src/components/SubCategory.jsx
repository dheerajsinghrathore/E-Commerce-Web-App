            ...
            <div className="p-6">
                {loading ? (
                    <div className="flex flex-col items-center justify-center min-h-[40vh] gap-4">
                        ...
                        <p className="text-neutral-500 font-medium animate-pulse">Fetching sub categories...</p>
                    </div>
                ) : subCategories.length > 0 ? (
                    <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {subCategories.map((sub) => (
                            <div key={sub._id} className="bg-white p-3 rounded-2xl shadow-sm border border-neutral-100 group hover:shadow-xl hover:shadow-primary-500/5 hover:-translate-y-1 transition-all duration-300 flex flex-col">
                                <div className="w-full h-32 bg-neutral-50 rounded-xl overflow-hidden mb-3 p-2">
                                    <img src={sub.image} alt={sub.name} className="w-full h-full object-scale-down group-hover:scale-110 transition-transform duration-500" />
                                </div>
                                <h3 className="font-bold text-neutral-800 text-center truncate px-2 mb-1">{sub.name}</h3>
                                <div className="flex flex-wrap gap-1 justify-center mb-3">
                                    {sub.category.map(cat => (
                                        <span key={cat._id} className="text-[10px] bg-primary-50 text-primary-600 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">{cat.name}</span>
                                    ))}
                                </div>
                    ...
                    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8 bg-white rounded-3xl border-2 border-dashed border-neutral-200 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <h3 className="text-2xl font-bold text-neutral-800 mb-2">No Sub Categories Found</h3>
                        <p className="text-neutral-500 max-w-sm mb-8 text-sm">Add your first sub category to start organizing your products better.</p>
                        <button onClick={() => setOpenSubCategory(true)} className="bg-primary-50 text-primary-600 hover:bg-primary-100 px-6 py-3 rounded-2xl font-bold transition-all active:scale-95 flex items-center gap-2">
